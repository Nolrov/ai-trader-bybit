def get_pa_breakout_retest_candidates():
    ...
    for lookback in [12, 20, 30]:
        for retest_tolerance in [0.0008, 0.0015]:
            for body_ratio in [0.45, 0.55]:
                for hold in [4, 8, 12]:
...
def apply_pa_breakout_retest(df, c):
    ...
    body_ratio = c.get("trigger_body_ratio", 0.5)

    range_high = df["high_15m"].rolling(lookback).max().shift(2)
    range_low = df["low_15m"].rolling(lookback).min().shift(2)
    bullish_trend = (df["ema_fast_30m"] > df["ema_slow_30m"]) & (df["ema_slope_fast_30m"] > 0)
    bearish_trend = (df["ema_fast_30m"] < df["ema_slow_30m"]) & (df["ema_slope_fast_30m"] < 0)

    breakout_long_prev = df["close_15m"].shift(1) > range_high
    breakout_short_prev = df["close_15m"].shift(1) < range_low

    long_retest = (
        breakout_long_prev
        & (df["low_15m"] <= range_high * (1 + tolerance))
        & (df["close_15m"] >= range_high)
        & (df["close_location_15m"] >= 0.58)
        & (df["body_ratio_15m"] >= body_ratio)
        & (df["lower_wick_ratio_15m"] >= 0.18)
    )
    short_retest = (
        breakout_short_prev
        & (df["high_15m"] >= range_low * (1 - tolerance))
        & (df["close_15m"] <= range_low)
        & (df["close_location_15m"] <= 0.42)
        & (df["body_ratio_15m"] >= body_ratio)
        & (df["upper_wick_ratio_15m"] >= 0.18)
    )
